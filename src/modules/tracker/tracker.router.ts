...
import { getTracker, getTrackerHistoryPaginated, postTracker } from "./tracker.controller.js";
...
trackerRouter.get("/tracker/history", requireUser, async (req: AuthedRequest, res) => {
  try {
    await getTrackerHistoryPaginated(req, res);
    return;
  } catch (err) {
    return jsonError(res, err);
  }
