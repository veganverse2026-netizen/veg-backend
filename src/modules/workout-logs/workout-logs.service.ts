...
async function markWorkoutCompletedForStreak(tx: Prisma.TransactionClient, userId: string) {
 ...
 */
export async function listPersonalRecordsPaginated(userId: string, page: number, limit: number, q?: string) {
  ...
  const [rows, countRows] = await Promise.all([
    prisma.$queryRaw<Array<{ exercise: string; bestWeightKg: number; bestVolume: number; updatedAt: Date }>>`
      SELECT * FROM (
...
async function notifyTrainerOfCompletion(userId: string) {
  const user = await prisma.user.findUnique({ where: { id: userId }, select: { name: true, gymTrainerId: true } });
  if (!user?.gymTrainerId) return;
  const trainer = await prisma.gymTrainer.findUnique({ where: { id: user.gymTrainerId }, select: { linkedUserId: true } });
  if (!trainer?.linkedUserId) return;
  await createNotification({
    userId: trainer.linkedUserId,
    type: "GYM",
    title: "Member completed a workout",
...
export async function getTodayWorkoutCompletion(userId: string) {
  const today = startOfDay(new Date());
...
export async function logWorkoutSession(
  userId: string,
  exercises: Array<{ exercise: string; sets: number; reps: number; weightKg: number; notes?: string | null }>
) {
  const today = startOfDay(new Date());
  const now = new Date();
  ...
  try {
    ...
    if (err?.code === "P2002") throw new HttpError(409, "Today's workout has already been completed");
    throw err;
  }
...
export async function markWorkoutDone(userId: string) {
  const today = startOfDay(new Date());
  const now = new Date();
