...
import { computeTDEE, evaluateGoalPace, computeGoalCalorieTarget, computeMacroTargets, computeHydrationTarget, predictWeeklyProgress } from "../../shared/domain/calorieEngine.js";
export async function postOnboarding(req, res) {
    ...
    if (!Number.isFinite(payload.weightKg) || payload.weightKg < 20 || payload.weightKg > 300)
        throw new HttpError(400, "Invalid weightKg");
    const result = await completeOnboarding(req.userId, payload);
    return jsonOk(res, result);
}
const GOAL_PREVIEW_GOALS = ["FAT_LOSS", "MUSCLE_BUILD", "LIFESTYLE"];
...
export async function postGoalPreview(req, res) {
    const body = requireObject(req.body);
    ...
    const gender = requireEnum(body, "gender", ["FEMALE", "MALE", "OTHER"]);
    const activityLevel = requireEnum(body, "activityLevel", ["SEDENTARY", "LIGHT", "MODERATE", "ACTIVE", "ATHLETE"]);
    const goal = requireEnum(body, "goal", GOAL_PREVIEW_GOALS);
    let targetWeightKg = null;
    let timelineWeeks = null;
    if (goal !== "LIFESTYLE") {
        const rawTarget = optionalNumber(body, "targetWeightKg");
        if (rawTarget === undefined || rawTarget < 20 || rawTarget > 300)
            throw new HttpError(400, "Invalid targetWeightKg");
        targetWeightKg = rawTarget;
        const rawWeeks = optionalNumber(body, "timelineWeeks");
        if (rawWeeks === undefined || !Number.isInteger(rawWeeks) || rawWeeks < 1 || rawWeeks > 104)
            throw new HttpError(400, "Invalid timelineWeeks");
